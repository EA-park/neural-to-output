...
# Each part's MJCF lives next to that part's own driver code (`robot/arm/so101/`,
# `robot/hand/amazing_hand_right/`), not under `simulation/` -- the hardware
# description belongs to the part, `simulation` only consumes it. `parents[2]` from
# this file (`simulation/mujoco/simulator.py`) is `robot/`.
_ROBOT_DIR = Path(__file__).parents[2]
_ARM_SCENE = _ROBOT_DIR / "arm" / "so101" / "mjcf" / "scene.xml"
_HAND_SCENE = _ROBOT_DIR / "hand" / "amazing_hand_right" / "mjcf" / "demo_scene.xml"
...
_HAND_ROBOT = _ROBOT_DIR / "hand" / "amazing_hand_right" / "mjcf" / "robot.xml"

# Not underscore-prefixed -- these two are real, reusable facts about how the
# arm+hand MJCF are welded together (which site, which rename prefix), not just
# this module's own implementation detail. `n2o.robot.simulation.unity.solver.
# ClosedLoopRigSolver`'s own `attach=` option (used by apps/console.py to build a
# merged rig.json for the Unity-native PhysX engine) imports these directly so
# both engines describe the identical physical rig instead of each hardcoding
# its own copy of the same site name/prefix.
ARM_GRIPPER_SITE = "gripperframe"
...
HAND_PREFIX = "hand_"
# `AmazingHand.GESTURES` pose entries (`src/n2o/robot/hand/amazing_hand_right/__init__.py`) are
...
class Simulator:
    ...
    def __init__(self, parts=("arm",), *, attach_hand_to_arm=False):
        ...
        # its own bare names.
        self._prefixes = {"arm": "", "hand": HAND_PREFIX if len(parts) > 1 else ""}
    ...
    def _build_model(self):
        ...
        if self._attach_hand_to_arm:
            site = arm_spec.site(ARM_GRIPPER_SITE)
            arm_spec.attach(hand_spec, prefix=HAND_PREFIX, site=site)
        else:
            frame = arm_spec.worldbody.add_frame(pos=_HAND_STANDALONE_OFFSET)
            arm_spec.attach(hand_spec, prefix=HAND_PREFIX, frame=frame)
    ...
    def _arm_ctrl(self, target_deg):
        ...
        from ...arm.so101.lerobot_robot_so101_5dof.solver import real_deg_to_mj_rad
