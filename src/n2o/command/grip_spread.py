...
"""Maps BNCI2014_001's four raw motor-imagery labels to four of `AmazingHand.GESTURES`'
poses -- see `GripSpreadCommand` below for why this mapping exists at all (no real
grip/release-labeled EEG dataset is bundled with this project). `feet`/`tongue` ->
`index_pointing`/`perfect` is an arbitrary but visually-distinct pick, same spirit as
`OfnerCommand.LABEL_TO_GESTURE` -- not a considered design."""
...
class GripSpreadCommand(Command):
    ...
    `tongue`) as stand-ins, mapped via `LABEL_TO_GESTURE` to four of `AmazingHand`'s
    named poses. A decoder trained on different labels needs a different `Command`
    subclass instead -- this one is specific to that label set.
